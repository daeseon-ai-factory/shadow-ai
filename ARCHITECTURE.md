...
> **정정 (2026-05-30):** AI 분석은 `AiAnalysisClient` 인터페이스로 **제공자 교체 가능**합니다. 기본은 **Google Gemini 2.5 Flash**(무료 티어), 대안으로 Claude Haiku 4.5 — `AI_PROVIDER` 환경변수로 선택. 두 클라이언트는 공용 `AiAnalysisParser`를 공유하고 중립 타입 `AiAnalysisResult`를 반환합니다. 아래에서 "Claude만" 쓰는 것처럼 서술된 부분은 이 추상화 도입 이전 내용이며, 오케스트레이션(트랜잭션 밖 비동기 호출 등) 설명 자체는 유효합니다.
