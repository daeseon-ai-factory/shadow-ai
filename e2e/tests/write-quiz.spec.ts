import { test, expect, Page, APIRequestContext } from "@playwright/test";

const BACKEND = "http://localhost:8080";

interface User { email: string; password: string; displayName: string; }
function freshUser(suffix = ""): User {
  const stamp = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  return { email: `e2e-${stamp}${suffix}@example.com`, password: "supersecret123", displayName: `E2E ${stamp}` };
}
async function signupEn(page: Page, user: User) {
  await page.goto("/en/signup");
  await page.getByLabel("Name").fill(user.displayName);
  await page.getByLabel("Email").fill(user.email);
  await page.getByLabel("Password").fill(user.password);
  await page.getByRole("button", { name: "Sign up" }).click();
  await page.waitForURL(/\/library/, { timeout: 15_000 });
}
async function loginViaApi(request: APIRequestContext, user: User): Promise<string> {
  const r = await request.post(`${BACKEND}/api/auth/login`, { data: { email: user.email, password: user.password } });
  expect(r.ok()).toBeTruthy();
  return (await r.json()).data.accessToken;
}
async function firstVideoId(request: APIRequestContext, token: string): Promise<string | null> {
  const r = await request.get(`${BACKEND}/api/collections/developer-picks`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!r.ok()) return null;
  const body = await r.json();
  const entries = body.data?.videos ?? [];
  return entries.find((e: { video: { transcriptStatus: string } }) =>
    e.video?.transcriptStatus === "READY")?.video?.id ?? entries[0]?.video?.id ?? null;
}

test.describe("Quiz Write mode", () => {
  test("review page can switch to Write, show Korean prompt, accept answer, reveal diff", async ({ page, request }) => {
    test.setTimeout(120_000);

    const user = freshUser("-wq");
    await signupEn(page, user);
    const token = await loginViaApi(request, user);
    const videoId = await firstVideoId(request, token);
    test.skip(!videoId, "no seeded video");

    // Create a clip and wait for analysis to be READY so the translation exists.
    const created = await request.post(`${BACKEND}/api/clips`, {
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      data: { videoId, startMs: 0, endMs: 8000, name: "Write test", tags: [] },
    });
    const clipId = (await created.json()).data.id;
    for (let i = 0; i < 30; i++) {
      const r = await request.get(`${BACKEND}/api/clips/${clipId}/analysis`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (r.ok() && (await r.json()).data.status === "READY") break;
      await new Promise((res) => setTimeout(res, 2000));
    }

    // Force this clip into the review queue.
    // Simplest path: go straight to /review — SM-2 puts new cards due immediately.
    await page.goto("/en/review");
    await expect(page.getByRole("heading", { name: /Write test|Review/i })).toBeVisible({ timeout: 15_000 });

    // Switch to Write mode
    await page.getByTestId("mode-write").click();

    // Korean prompt appears
    const writeInput = page.getByTestId("write-input");
    await expect(writeInput).toBeVisible({ timeout: 10_000 });

    // Type a wrong-but-similar answer to exercise the diff
    await writeInput.fill("All right who is ready for category theory");
    await page.getByTestId("write-check").click();

    // Diff appears
    await expect(page.getByTestId("diff-your")).toBeVisible();
    await expect(page.getByTestId("diff-expected")).toBeVisible();
    await page.screenshot({ path: "test-results/write-quiz-diff.png", fullPage: true });

    // Switch back to Reveal still works
    await page.getByTestId("mode-reveal").click();
    await expect(page.getByRole("button", { name: /Show subtitle|자막 보기/i })).toBeVisible();
  });
});
